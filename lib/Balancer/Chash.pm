...
sub crc32_update(\$\@$);
sub chash_point_init_crc(\@$$$$$);
sub chash_point_init(\@$$$$);
sub chash_point_sort(\@$);
sub chash_point_add(\@$$$$$\@);
sub chash_point_reduce(\@$$$$$);
sub chash_point_delete(\@$$);
...
sub chash_point_init_crc(\@$$$$$) {
    my ($_arr, $start, $base_hash, $from, $num, $id) = @_;
    ...
    for (my $i = 0; $i < $from + $num; $i++) {
        ...
        $prev_hash->{byte}[0] = $hash & 0xff;
        $prev_hash->{byte}[1] = ($hash >> 8) & 0xff;
        $prev_hash->{byte}[2] = ($hash >> 16) & 0xff;
        $prev_hash->{byte}[3] = ($hash >> 24) & 0xff;
    }
}
...
sub chash_point_add(\@$$$$$\@) {
    my ($_old_points, $old_length, $base_hash, $from, $num, $id, $_new_points)
        = @_;

    my ($i, $j, $k);
    my $tmp_points = calloc_chash_point_t($num);
    ...
    for ($i = $old_length - 1; $i >= 0; $i--, $k--) {
        while ($j >= 0
            && $tmp_points->[$j]->{hash} > $_old_points->[$i]->{hash})
        {
...
sub chash_point_reduce(\@$$$$$) {
    my ($_old_points, $old_length, $base_hash, $from, $num, $id) = @_;
    ...
    for ($i = 0, $j = 0, $k = 0; $i < $old_length; $i++) {
        if (   $j < $num
            && $_old_points->[$i]->{hash} == $tmp_points->[$j]->{hash}
            && $_old_points->[$i]->{hash} == $tmp_points->[$j]->{id})
        {
            $j++;
            next;
        }
        ...
        if ($i != $k) {
            $_old_points->[$k]->{hash} = $_old_points->[$i]->{hash};
            $_old_points->[$k]->{id}   = $_old_points->[$i]->{id};
        }
...
sub chash_point_delete(\@$$) {
    my ($_old_points, $old_length, $id) = @_;

    my ($i, $j);

    for ($i = 0, $j = 0; $i < $old_length; $i++) {
        if ($_old_points->[$i]{id} == $id) {
            next;
        }

        if ($i != $j) {
            $_old_points->[$j]->{hash} = $_old_points->[$i]->{hash};
            $_old_points->[$j]->{id}   = $_old_points->[$i]->{id};
        }
